"""Smoke-тест для get_video_ads_daily_stat() — реальный API."""
...
from ozon_performance import get_video_ads_daily_stat, VIDEO_ADS_COLUMNS  # noqa: E402
...
df = get_video_ads_daily_stat(date_from, date_to, raw_cache_dir=RAW_CACHE_DIR)

# Assertions
assert list(df.columns) == VIDEO_ADS_COLUMNS, f"Колонки не совпадают: {list(df.columns)}"
...
out_path = PROJECT_ROOT / "raw_data" / f"get_video_ads_daily_stat_{date_from}_{date_to}.csv"
