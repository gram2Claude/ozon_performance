...
- **Кодировка CSV:** UTF-8 BOM (`\xef\xbb\xbf`) — стрипать перед парсингом.
- **Разделитель CSV:** `;` (не `,`).
...
Гранулярность: одна строка на `ad_id × platform × date`.
**Нельзя агрегировать по платформам** — уникальные пользователи между платформами пересекаются.

Тот же запрос что 3.4 (`groupBy=NO_GROUP_BY`), тот же кэш.
Отличие: брать ad-level строки CSV (НЕ строку «Всего»).
...
| `reach` | float | `Охват` |
| `increment` | float | `reach[D] − reach[D−1]` по группе `(campaign_id, ad_id, platform)` |
...
Для охват-функций (3.4, 3.5):
   ...
   После сбора: вычислить increment через diff по (campaign_id) или (campaign_id, ad_id, platform).
```
...
- [ ] Числа парсятся корректно (запятая → точка), BOM стрипается,
