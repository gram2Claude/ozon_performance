...
Плейсхолдеры:
  ...
  {ENV_VAR_1}         — переменная окружения для первого credentials (OZON_CLIENT_ID)
  {ENV_VAR_2}         — переменная окружения для второго credentials (OZON_CLIENT_SECRET)
...
## 2. Ключевые принципы работы с {API_NAME}
...
<!-- [CUMULATIVE] -->
### 2.7. Особый случай — кумулятивная метрика «{CUMULATIVE_METRIC_NAME}»
...
### 3.2. {FUNCTION_2_NAME}(date_from, date_to)
...
<!-- Пример:
### 3.3. {FUNCTION_3_NAME}(date_from, date_to)
...
<!-- [CUMULATIVE] -->
### 3.{FN_CUMULATIVE_NUM}. {FUNCTION_CUMULATIVE_NAME}(global_start_date, date_from, date_to)
...
### 6.1. {FUNCTION_1_NAME} — справочник
...
### 6.2. {FUNCTION_2_NAME} — дневная статистика
...
<!-- [CUMULATIVE] -->
### 6.3. {FUNCTION_CUMULATIVE_NAME} — кумулятивная метрика
...
## 7. Рекомендации по реализации на {TARGET_LANG}
