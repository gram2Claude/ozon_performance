...
(показы, клики, расход, охват, видео-метрики), плюс строит сводную таблицу-агрегат
`admin_audit`.
...
### 4.7. `get_admin_audit(date_from, date_to)` — сводный аудит (таблица admin_audit)

Собственного эндпоинта в API нет — **агрегат поверх 4.2** (`get_campaigns_daily_stat`):
к строкам статистики кампаний джойнится `owner_id` из справочника кампаний (4.1, по
`campaign_id`), затем метрики **суммируются** по ключу
`date × account_id × source_type_id × owner_id`. 9 колонок:
...
Наследует поведение и лимиты 4.2 (статистика по дням, батчи по 10, общий кэш
`raw_{...}.csv`). Пустая статистика → пустой результат с этими колонками.
...
### 7.7. `get_admin_audit` — сводный аудит (таблица admin_audit)
...
*(Все кампании аккаунта имеют `owner_id = 1` → одна строка на день.)*
...
- **Тесты**: моки HTTP-клиента на все 7 публичных функций (включая агрегат `get_admin_audit`) +
...
- [ ] Все 7 публичных функций возвращают данные с теми же колонками
