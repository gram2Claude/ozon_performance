...
<!-- Пример: «Reach — кумулятивный показатель (уникальные пользователи за период).
Для корректного получения дневного значения каждый день запрашивается отдельно как период [global_start_date, D].» -->
...
<!-- Для кумулятивной метрики добавь строку:
| increment | float | Прирост метрики относительно предыдущей даты; для первой даты — NaN |
-->
