...
### 3.8. TLS / проверка сертификатов сервера

С 2026 года Ozon меняет корневой удостоверяющий центр (УЦ) TLS-сертификатов
своих серверов: **GlobalSign → Harica**. Это плановое обновление затрагивает
все публичные API Ozon (Performance API, Seller API и др.).

Требования к клиенту:
- **Доверять актуальному CA-бандлу**, а не «жёстко зашитому» (hardcoded) списку
  корневых сертификатов. Бандл должен содержать корень Harica (актуальные бандлы
  уже содержат и Harica, и GlobalSign — переход проходит бесшовно).
- **Не пинить сертификат** и не указывать устаревший собственный `ca-bundle` —
  иначе после миграции соединение упадёт с ошибкой
  `certificate verify failed` / `unable to get local issuer certificate`.
- **Не отключать проверку сертификата** (`verify=false`) как «обход» проблемы —
  это создаёт уязвимость (MITM), а не восстанавливает доверие. Правильное
  решение — обновить CA-бандл.
- Держать CA-бандл обновляемым: в окружениях с автообновлением системного
  хранилища ОС это происходит само; при собственном бандле — обновлять его
  (детали для PHP — раздел 8).
...
- **TLS / CA-бандл**: Guzzle/cURL по умолчанию проверяют сертификат сервера по
  системному CA-бандлу. С 2026 Ozon переходит на корневой УЦ **Harica** (с GlobalSign) —
  убедиться, что используемый бандл актуален и содержит корень Harica. **Не** задавать
  устаревший `CURLOPT_CAINFO` / `'verify' => '/path/old/cacert.pem'` и **не** отключать
  проверку (`'verify' => false`). При ошибке `certificate verify failed` /
  `unable to get local issuer certificate` — обновить бандл
  (`composer require composer/ca-bundle` или системные `ca-certificates`), а не отключать verify.
...
- [ ] Соединение с API использует актуальный CA-бандл (доверие к корню Harica);
      проверка TLS-сертификата не отключена.
