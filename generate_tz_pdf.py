...
P(
    ...
    "(показы, клики, расход, охват, видео-метрики), плюс строит сводную таблицу-агрегат admin_audit. "
...
H2("3.8. TLS / проверка сертификатов сервера")
P(
    "С 2026 Ozon меняет корневой удостоверяющий центр (УЦ) TLS-сертификатов своих "
    "серверов: GlobalSign -> Harica. Плановое обновление затрагивает все публичные "
    "API Ozon (Performance API, Seller API и др.)."
)
BUL([
    "Доверять актуальному CA-бандлу, а не «жёстко зашитому» (hardcoded) списку корней. "
    "Бандл должен содержать корень Harica (актуальные бандлы содержат и Harica, и GlobalSign — "
    "переход бесшовный).",
    "Не пинить сертификат и не указывать устаревший собственный ca-bundle — иначе после "
    "миграции соединение упадёт: certificate verify failed / unable to get local issuer certificate.",
    "Не отключать проверку сертификата (verify=false) как «обход» — это уязвимость (MITM), "
    "а не восстановление доверия. Правильно — обновить CA-бандл.",
    "Держать CA-бандл обновляемым: с автообновлением системного хранилища ОС это происходит "
    "само; при собственном бандле — обновлять его (детали для PHP — раздел 8).",
])
...
H2("4.7. get_admin_audit(date_from, date_to) — сводный аудит (таблица admin_audit)")
P(
    "Собственного эндпоинта в API нет — агрегат поверх 4.2 (get_campaigns_daily_stat): "
    "к строкам статистики кампаний джойнится owner_id из справочника (4.1, по campaign_id), "
    "затем метрики суммируются по ключу date × account_id × source_type_id × owner_id. 9 колонок:"
)
TABLE(
    ["Поле", "Тип", "Источник"],
    [
        ["date", "string YYYY-MM-DD", "из статистики кампаний (4.2)"],
        ["account_id", "integer", "константа (значение 1 — пример)"],
        ["source_type_id", "integer", "константа (значение 9 — пример)"],
        ["owner_id", "integer", "из справочника кампаний (4.1), join по campaign_id"],
        ["impressions", "float", "сумма по группе"],
        ["clicks", "float", "сумма по группе"],
        ["costs_nds", "float", "сумма по группе (округление до 2 знаков)"],
        ["costs_without_nds", "float", "сумма по группе (округление до 2 знаков)"],
        ["chef_flag", "integer", "константа (значение 1 — дефолт)"],
    ],
    [40, 25, 115],
)
P("Наследует поведение и лимиты 4.2 (статистика по дням, батчи по 10, общий кэш raw_*.csv). "
  "Пустая статистика → пустой результат с этими колонками.")
...
H2("7.7. get_admin_audit — сводный аудит (таблица admin_audit)")
TABLE(
    ["date", "account_id", "source_type_id", "owner_id", "impressions", "clicks", "costs_nds", "costs_without_nds", "chef_flag"],
    [
        ["2026-04-24", "1", "9", "1", "456002", "273", "136801.0", "112131.97", "1"],
        ["2026-04-25", "1", "9", "1", "455250", "256", "136575.0", "111946.72", "1"],
    ],
    [22, 16, 24, 16, 22, 14, 20, 28, 16],
)
P("Все кампании аккаунта имеют owner_id = 1 → одна строка на день. "
  "Значения account_id, source_type_id, chef_flag приведены как пример.")
...
BUL([
    ...
    "TLS / CA-бандл: Guzzle/cURL по умолчанию проверяют сертификат сервера по системному "
    "CA-бандлу. С 2026 Ozon переходит на корневой УЦ Harica (с GlobalSign) — убедиться, что "
    "бандл актуален и содержит корень Harica. Не задавать устаревший CURLOPT_CAINFO / "
    "'verify' => '/path/old/cacert.pem' и не отключать проверку ('verify' => false). При ошибке "
    "certificate verify failed — обновить бандл (composer require composer/ca-bundle или "
    "системные ca-certificates), а не отключать verify.",
    ...
    "Тесты: моки HTTP на все 7 функций (включая агрегат get_admin_audit) + retry на 429 + таймаут polling.",
])
...
BUL([
    "Все 7 функций возвращают данные с теми же колонками и в том же порядке, "
    "что в Python-эталоне ozon_performance.py.",
    "get_admin_audit суммирует метрики по date × account_id × source_type_id × owner_id, "
    "проставляет chef_flag = 1, деньги округляет до 2 знаков.",
